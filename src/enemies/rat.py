...
class RatEnemy(Enemy):
    ...
    def __init__(self, pos: tuple[int, int]):
        ...
        idle_sheet = load_image("rat-idle.png")
        self.anim_idle = Animation(slice_sprite_sheet_row(
            idle_sheet, row=0, frame_w=32, frame_h=32, num_frames=6, stride_x=64, start_x=0, start_y=0, clamp=True
        ), frame_duration=0.15, loop=True) 

        run_sheet = load_image("rat-run.png")
        self.anim_run = Animation(slice_sprite_sheet_row(
            run_sheet, row=0, frame_w=32, frame_h=32, num_frames=6, stride_x=64, start_x=0, start_y=0, clamp=True
        ), frame_duration=0.10, loop=True)

        attack_sheet = load_image("rat-attack.png")
        self.anim_attack = Animation(slice_sprite_sheet_row(
            attack_sheet, row=0, frame_w=32, frame_h=32, num_frames=6, stride_x=64, start_x=0, start_y=0, clamp=True
        ), frame_duration=0.10, loop=False)

        die_sheet = load_image("rat-death.png")
        self.anim_die = Animation(slice_sprite_sheet_row(
            die_sheet, row=0, frame_w=32, frame_h=32, num_frames=6, stride_x=64, start_x=0, start_y=0, clamp=True
        ...
        self.base_speed = 100.0 # speed here

        self.health = 20 # health here
