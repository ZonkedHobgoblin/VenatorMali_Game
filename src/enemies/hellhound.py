...
class HellHoundEnemy(Enemy):
    ...
    def __init__(self, pos: tuple[int, int]):
        ...
        idle_sheet = load_image("hell-hound-idle.png")
        self.anim_idle = Animation(slice_sprite_sheet_row(
            idle_sheet, row=0, frame_w=55, frame_h=32, num_frames=6, stride_x=64, start_x=11, start_y=0, clamp=True
        ), frame_duration=0.15, loop=True) 

        run_sheet = load_image("hell-hound-run.png")
        self.anim_run = Animation(slice_sprite_sheet_row(
            run_sheet, row=0, frame_w=55, frame_h=32, num_frames=5, stride_x=64, start_x=11, start_y=0, clamp=True
        ), frame_duration=0.10, loop=True)

        attack_sheet = load_image("hell-hound-jump.png")
        self.anim_attack = Animation(slice_sprite_sheet_row(
            attack_sheet, row=0, frame_w=55, frame_h=48, num_frames=6, stride_x=64, start_x=11, start_y=0, clamp=True
        ), frame_duration=0.10, loop=False)

        die_sheet = load_image("hell-hound-idle.png")
        self.anim_die = Animation(slice_sprite_sheet_row(
            die_sheet, row=0, frame_w=55, frame_h=32, num_frames=6, stride_x=64, start_x=11, start_y=0, clamp=True
        ), frame_duration=0, loop=False)
        ...
        self.base_speed = 130.0 # speed here

        self.health = 25 # health here
